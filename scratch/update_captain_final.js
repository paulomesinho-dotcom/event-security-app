...
// 3. Fetch allPlans in useEffect
const workplacesSnapTarget = 'const unsubWorkplaces = onSnapshot(collection(db, "workplaces"), (snap) => {';
const newPlansSnap = `const unsubPlans = onSnapshot(collection(db, "plans"), (snap) => {
      setAllPlans(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    const unsubWorkplaces = onSnapshot(collection(db, "workplaces"), (snap) => {`;

if (!content.includes('const unsubPlans = onSnapshot')) {
  content = content.replace(workplacesSnapTarget, newPlansSnap);
  content = content.replace('unsubNotifs(); unsubFcm(); unsubWorkplaces();', 'unsubPlans(); unsubNotifs(); unsubFcm(); unsubWorkplaces();');
}

// 4. Compute workplacePlans
const activeWpCalcEnd = 'activeWorkplace = workplaces.find(w => w.id === user.workplaceId) || null;\n  }';
const newWpPlansCalc = `activeWorkplace = workplaces.find(w => w.id === user.workplaceId) || null;
  }
...
if (!content.includes('const workplacePlans =')) {
  content = content.replace(activeWpCalcEnd, newWpPlansCalc);
}

// 5. Insert send functions
...
// 6. Replace Comunicações panel UI
  ...
  if (headerIndex !== -1) {
    ...
    if (startIndex !== -1) {
      ...
      if (capNotConfig !== -1) {
        ...
        const closeContainer = content.indexOf('</div>', closeIf) + 6;
...
// 7. Insert new Modals conditioned on activePanel === 'zello'
...
console.log('Successfully applied final enhancements to CaptainPatrolDashboard.tsx');
