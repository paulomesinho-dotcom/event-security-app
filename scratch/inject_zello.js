...
function injectZelloPanel(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');

  // If the file already has the Zello modal, skip or replace
  if (!content.includes('activePanel === "zello"')) {
    console.log('Skipping ' + filePath + ' (no Zello panel button)');
    return;
  }

  // Add state for global notification modal
  if (!content.includes('const [showGlobalAlertModal, setShowGlobalAlertModal]')) {
    content = content.replace(/const \[activePanel, setActivePanel\] = useState<string \| null>\(null\);/, 
...
`;
  ...
  // Add the panels just after the other activePanels (e.g., info or suspects)
  if (!content.includes(`activePanel === 'zello' && !showGlobalAlertModal`)) {
    const panelsHTML = `
...
`;
    ...
    if (bottomBarIndex !== -1) {
      content = content.slice(0, bottomBarIndex) + panelsHTML + '\n      ' + content.slice(bottomBarIndex);
    }
  }

  // Update VigiaDashboard's bottom bar to use setActivePanel instead of a direct link
  if (filePath.includes('VigiaDashboard.tsx')) {
    // Replace the direct a href for Zello with a button that toggles activePanel
    const regex = /\{zelloLink && \([\s\S]*?<a href=\{zelloLink\}[\s\S]*?<Play size=\{20\} fill="currentColor" \/>[\s\S]*?<\/div>\s*<span style=\{\{ fontSize: "0\.65rem", fontWeight: 600, color: "#f97316" \}\}>Rádio<\/span>\s*<\/a>\s*\)\}/;
    
    // We remove {zelloLink && (...)} and just make it a button since we want the panel to open even if there is no zello link (so they can send the global notification!)
    ...
    // Also change Play icon to Radio icon inside CaptainPatrol if needed, but it's already using Radio.
    // Wait, Vigia uses <Play> but we imported <Radio>? Let's import Bell and Radio if not imported.
  }
  ...
  console.log('Injected Zello panel into ' + filePath);
}

injectZelloPanel(path.join(__dirname, '../src/components/CaptainPatrolDashboard.tsx'));
injectZelloPanel(path.join(__dirname, '../src/components/VigiaDashboard.tsx'));
