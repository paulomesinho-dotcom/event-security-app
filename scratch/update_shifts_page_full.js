...
export default function ShiftsPage() {
  ...
  useEffect(() => {
    ...
    const unsubShifts = onSnapshot(query(collection(db, "shifts"), where("captainId", "==", user.uid)), snapShifts => {
            ...
            <tr>
              {currentTab.dates.map(d => (
                 currentTab.periods.map(p => (
                   <th key={\\-\\} style={{ ...thStyle, fontSize: "0.8rem", textAlign: "center", background: "var(--color-bg)" }}>
            ...
            return (
              <tr key={locator.id} style={{ borderBottom: "1px solid var(--color-border)" }}>
                ...
                {currentTab.dates.map(d => (
                    ...
                    return (
                      <td key={\\-\-\\} style={{ ...tdStyle, textAlign: "center", verticalAlign: "top" }}>
                        {!shiftTime ? (
